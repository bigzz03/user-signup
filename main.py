...
@app.route("/")
def index():
    return render_template('home.html', title="Sign up Form")
...
def Sign_up():
    ...
    if password.count(space) != 0:
        ...
        match = ''
    if match != password:
        match_error = "Password and confirmed password must match."
        password = ''
        match = ''
...
def welcome():
    ...
    return render_template('welcome.html', title="Sign up Successful", user=user)
